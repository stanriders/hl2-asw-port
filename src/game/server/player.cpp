...
int CBasePlayer::ShouldTransmit( const CCheckTransmitInfo *pInfo )
{
	...
	if ( (IsAlive() && IsEffectActive( EF_NODRAW )) || ( IsObserver() && ( gpGlobals->curtime - m_flDeathTime > 0.5 ) && 
...
//-----------------------------------------------------------------------------
// Purpose: Pretty cheat...isnt it?
//-----------------------------------------------------------------------------
static void CreateGlobalLight( CBasePlayer *pPlayer )
{
 // Cheat to create a jeep in front of the player
 Vector vecForward;
 AngleVectors( pPlayer->EyeAngles(), &vecForward );
 CBaseEntity *pGlobalLight = ( CBaseEntity* )CreateEntityByName( "env_global_light" );
 if ( pGlobalLight )
 {
  Vector vecOrigin = pPlayer->GetAbsOrigin() + vecForward * 256 + Vector( 0,0,64 );
  QAngle vecAngles( 0, pPlayer->GetAbsAngles().y - 90, 0 );
  pGlobalLight->SetAbsOrigin( vecOrigin );
  pGlobalLight->SetAbsAngles( vecAngles );
  pGlobalLight->KeyValue( "xoffset", "0" );
  pGlobalLight->KeyValue( "yoffset", "165" );
  pGlobalLight->KeyValue( "brightnessscale", "0.7" );
  pGlobalLight->KeyValue( "cascades", "3" );
  pGlobalLight->KeyValue( "color", "237 218 143 1" );
  pGlobalLight->KeyValue( "colortransitiontime", "0.5" );
  pGlobalLight->KeyValue( "distance", "12000" );
  pGlobalLight->KeyValue( "enableshadows", "1" );
  pGlobalLight->KeyValue( "fov", "10" );
  pGlobalLight->KeyValue( "nearz", "1536" );
  pGlobalLight->KeyValue( "northoffset", "165" );
  pGlobalLight->KeyValue( "scale", "2" );
  pGlobalLight->KeyValue( "texturename", "effects/flashlight001" );
  pGlobalLight->KeyValue( "targetname", "globallight" );
  DispatchSpawn( pGlobalLight );
  pGlobalLight->Activate();
 }
}
void CC_CH_CreateGlobalLight( void )
{
 CBasePlayer *pPlayer = UTIL_GetCommandClient();
 if ( !pPlayer )
  return;

 CreateGlobalLight( pPlayer );
}

static ConCommand ch_create_globallight( "ch_create_globallight", CC_CH_CreateGlobalLight, "Spawn a global light.", FCVAR_CHEAT );
...
{
	m_Local.m_vecPunchAngle.Set( axis, value );
	...
	if ( IsAlive() )
	{
		...
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if ( pPlayer && i != index && pPlayer->GetObserverTarget() == this && pPlayer->GetObserverMode() == OBS_MODE_IN_EYE )
			{
				pPlayer->SetPunchAngle( GetPunchAngle() );
			}
		}
	}
}
